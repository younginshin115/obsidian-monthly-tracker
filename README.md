...
파일명이 `YYYY-MM-DD`로 시작해야 합니다.
`2026-06-03.md`, `2026-06-03 Tuesday.md` 모두 인식됩니다.
...
`year`, `month`는 노트의 frontmatter에서 자동으로 읽어옵니다.
...
#### [boolean] 파일 존재 여부 — `property: null`

파일이 존재하기만 하면 색칠됩니다. frontmatter 없이 파일만 만들어도 기록이 남는 경우에 유용합니다.
...
| `property` | ✅ | 일간 노트의 frontmatter 키 |
| `preset` | | 내장 프리셋 이름 |
| `colors` | | 값 → 색상 직접 지정 (`preset`과 둘 중 하나) |
| `source` | | 폴더 경로 오버라이드 |

#### [colormap] 컨디션 — 내장 `condition` 프리셋

`good` / `soso` / `tired` / `bad` 네 가지 값을 자동으로 색상에 매핑합니다.
...
숫자 값을 강도 단계로 표시합니다. 수면 시간, 운동 거리, 독서 페이지 등에 적합합니다.
...
|------|------|------|
| `type` | ✅ | `heatmap` |
| `property` | ✅ | 일간 노트의 frontmatter 키 (숫자여야 함) |
| `colorScheme` | | 내장 색상 스킴 이름 |
| `colors` | | 색상 배열 직접 지정 (길이 = `bins.length + 1`) |
| `bins` | | 구간 기준값 배열 (기본값: `[1, 3, 5, 7]`) |
| `unit` | | 단위 문자열 (툴팁 및 합계에 표시, 예: `km`, `h`, `분`) |
| `showTotal` | | 트래커 아래 월 합계 표시 여부 |
...
Heatmap의 각 스킴은 연한 색(데이터 없음/낮음)부터 진한 색(높음)까지 6단계로 구성됩니다. 기본값은 `indigo`.

### Colormap `preset`

| 이름 | 값 |
|------|-----|
| `condition` | `good` (파랑), `soso` (초록), `tired` (주황), `bad` (빨강) |
...
트래커별로 `source` 옵션을 써서 폴더를 오버라이드할 수 있습니다:
...
**셀에 색이 안 칠해짐** — `property` 이름이 일간 노트 frontmatter 키와 정확히 일치하는지, 파일이 설정된 폴더 안에 `YYYY-MM-DD`로 시작하는 이름으로 있는지 확인하세요.
